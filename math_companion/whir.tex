...
For iteration \(i\in[M]\) and folding round \(s\in[k]\) in JBR we do\footnote{The function \texttt{pcs/whir.py:\_epsilon\_fold()}
...
\[
  S_{\mathrm{sumcheck}} = k(d-1)\bfield.
\]
...
\[
  S_{\mathrm{ood},i}
    = \bhash+w_i\bfield+k(d-1)\bfield
\]

$$
S_{\mathrm{ood},M}=2^{m_M}\bfield
$$
...
\[
  S_{\mathrm{qry},i}
    =\begin{cases}  eMP(2^{m_i+\mu_i-k},t_i,2^k,\bfield,\bhash).\quad i>0\\
     eMP(2^{m_0+\mu_0-k},t_0, B\cdot 2^k,\bprime,\bhash).\quad i=0
    \end{cases}
\]
