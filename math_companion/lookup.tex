...
We capture case $(i)$ when proven with GKR. Following Papini and Haböck~\cite{logup-GKR} we calculate:

$$\epsilon_{\mathsf{logup-sound}}=\epsilon_{\mathsf{GKR}}(H_{\log(H)+\log(M)})\leq \frac{1}{2}\frac{(\log(H)+\log(M))(3(\log(H)+\log(M))+1)}{F}$$
...
Note that by combining the columns of $L$ and $T$ with multilinear coefficients $\text{eq}(\vec{Y}, \cdot)$ instead of powers of $Y$, it is possible to achieve a multiplicative overhead of $\log_2 S$ instead of $S$.
...
For both cases we capture the option to decrease the soundness error of the lookup round through grinding. We consider a parameter $\mathsf{grinding\_bits\_lookup}$ denoting the rounds of proof of work, and compute the soundess error as: $\epsilon=(\epsilon_{\mathsf{sum}}+\epsilon')2^{\mathsf{-grinding\_bits\_lookup}}$, where $\epsilon'$ can be $0$, $\epsilon_{\mathsf{logup-sound}}$, $\epsilon_{\mathsf{reduction-univ}}$, or the GKR soundness error $\epsilon_{\mathsf{gkr}}.$
