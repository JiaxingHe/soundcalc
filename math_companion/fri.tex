...
\begin{itemize}
 %   \item $m_J$ --- Johnson parameter.
    ...
    \item $\sbtch$ --- \emph{batch size}. Number of functions appearing in the batched-FRI.
...
The security level is calculated separately for two regimes: UDR and JBR, using the same procedure.\footnote{See \texttt{pcs/fri.py:get\_pcs\_security\_levels()}.}
\begin{enumerate}
...
The batching error\footnote{See \texttt{pcs/fri.py:\_get\_batching\_error()}.} uses the functions from \cref{sec:batch}.

In the Johnson Bound Regime (JBR) we compute the base error as:
\[
\ebatch^{\text{base,JBR}} =
\begin{cases}
...
\begin{cases}
  ...
  \ebatchlinU(\rho,N,q)& \text{(linear batching)}, \\[4pt]
  \ebatchmulU(\rho,N,q,B)& \text{(multilinear batching)}.
\end{cases}
\]

\subsection{Commit phase errors}

For round\footnote{See \texttt{pcs/fri.py:\_get\_commit\_phase\_error()}.} $i\in[r_{FRI}]$ we compute the dimension $N_i$ as
$$
...
The query phase error\footnote{See \texttt{pcs/fri.py:\_get\_query\_phase\_error()}.} is:
\begin{equation}
...
The FRI proof\footnote{See \texttt{pcs/fri.py:get\_FRI\_proof\_size\_bits()}.} contains two parts: Merkle roots, and one "openings" per query,
...
\begin{multline}
    \bproof =   \underbrace{ \bhash+  eMP(n,  t ,\sbtch,\bfield,\bhash)}_{\text{Initial round}} + \underbrace{\sum_{0\leq i < r_{FRI} } \left(\bhash+
      eMP(\frac{n}{\prod_{0\leq j \leq  i} \folds[j]},  t ,\sbtch,\bfield,\bhash)\right)}_{\text{Folding rounds }}
    \end{multline}
where $eMP(n, t, b, \bfield, \bhash)$ is the expected Merkle path size for $t$ queries to the $n$ leaf tree,
where each leaf has $b$ elements of $\bfield$ bits each,
calculated as\footnote{See \texttt{common/utils.py:get\_size\_of\_merkle\_multi\_proof\_bits\_expected()}.}
\begin{equation}
    eMP(n,t, b,\bfield,\bhash) =   \underbrace{t\cdot b\cdot \bfield}_{\text{ size of all}}
    +\bhash\sum_{1\leq d\leq \lceil \log_{2} n\rceil}\lceil 2^d\left((1-2^{-d})^t-(1-2^{1-d})^t\right)\rceil
\end{equation}